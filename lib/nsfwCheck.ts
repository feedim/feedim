...
export function evaluateNsfwScores(
  ...
  if (porn >= t.porn) return { action: 'flag', reason: 'Pornographic content detected in image.' };
  if (hentai >= t.hentai) return { action: 'flag', reason: 'Hentai content detected in image.' };
  if (sexy >= t.sexy) return { action: 'flag', reason: 'Erotic content detected in image.' };
  if (porn + hentai >= t.pornHentaiSum) return { action: 'flag', reason: 'Sexual content detected in image.' };
  if (porn + hentai + sexy >= t.totalSum && neutral < t.neutralMax) return { action: 'flag', reason: 'Inappropriate content detected in image.' };
...
export async function checkNsfwBuffer(
  ...
  try {
    ...
    if (!tensor) {
      return { safe: false, action: 'flag', scores: {}, reason: 'Image could not be decoded' };
    }
    ...
    try {
      ...
      logServerError('[NSFW] classify error', err, {
        operation: 'classify_image',
      });
      tensor.dispose();
      return { safe: false, action: 'flag', scores: {}, reason: 'Model classification error' };
    }
  } catch {
    return { safe: false, action: 'flag', scores: {}, reason: 'NSFW module error' };
  }
}
...
export async function checkNsfwContent(htmlContent: string): Promise<NsfwContentResult> {
  ...
  try {
    ...
    logServerError('[NSFW] Content check failed, allowing', err, {
      operation: 'check_content_batch',
    });
