...
[spam_scam] Bahis/kumar reklam, garanti kazanç, MLM, sahte çekiliş, phishing → flag. Anlamsız karakter dizisi, random metin, tekrarlayan içerik, test içerik → spam DEĞİL, ALLOW.
...
- NSFW görseli zaten ayrıca taranıyor. if: satırında "flagged" varsa görsel NSFW'dir, ek analiz gerekmez.
- Profil bio'da tekrarlanan karakter, dekoratif Unicode, emoji dizisi, estetik sembol, anlamsız karakter tekrarı → spam DEĞİL, ALLOW. Bunlar kullanıcı tarzıdır, moderasyon sebebi değildir.
- ct:profile ise flag'lenecekler: ağır küfür/hakaret, bahis/kumar, spam/dolandırıcılık, cinsel içerik, kişisel veri (IBAN/kart/TC), siyasi propaganda, dini değerlere hakaret. Anlamsız metin, random karakter, tekrarlanan metin, dekoratif Unicode, platform yönlendirme → profillerde ALLOW.

OUTPUT (JSON only, no markdown):
Flag: {"action":"flag","category":"<slug>","severity":"<low|medium|high|critical>","reason":"<Kısa Türkçe, max 80 char>","confidence":<0.0-1.0>}
Allow: {"action":"allow"}

reason: Sadece ihlali basitçe belirt, dilbilgisi analizi yapma. Cümle nokta ile bitsin. Hiçbir zaman skor değeri, yüzde veya teknik parametre yazma.
...
export type ModerationMeta = {
  contentType?: 'post' | 'video' | 'moment' | 'note' | 'comment' | 'profile' | 'metadata';
  linkCount?: number;
  imageHint?: string;
  profileScore?: number;
  spamScore?: number;
...
const SPAM_PATTERNS: Array<{ re: RegExp; reason: string }> = [
  ...
  { re: /(garanti kazanç|%100 kazanç|günde \d+ tl|anında para|hızlı zengin|mlm|piramit|ponzi)/i, reason: 'Spam: scam/fraud' },
  { re: /(takipçi\s*(kazan|sat|al|artır)|abone\s*(kazan|sat|al)|like\s*(kazan|sat))/i, reason: 'Spam: fake engagement' },
  { re: /(bahis|casino|kumar|slot|bet|1xbet|bwin|betboo|superbahis|youwin|mobilbahis)/i, reason: 'Spam: gambling' },
  { re: /(tipobet|rivalo|betnano|canlı\s*bahis|bahis\s*sitesi|bahis\s*oranı)/i, reason: 'Gambling site' },
  { re: /(bedava para|kolay para|günde \d+ tl kazan|yatırımsız|risksiz kazanç)/i, reason: 'Money promise fraud' },
  { re: /(profilimdeki\s*link|bio.*link|bio'mdaki|linke\s*tıkla|link\s*in\s*bio|biyografim|profil\s*linkime)/i, reason: 'Platform redirect' },
  { re: /(dm['']den\s*yaz|dm\s*at|iletişim\s*için\s*dm)/i, reason: 'Platform redirect' },
  { re: /(t\.me\/|telegram\.me\/|discord\.gg\/)/i, reason: 'External platform redirect' },
  { re: /(iban|banka\s*hesap|kart\s*bilgi|kredi\s*kart\s*no|cvv\b|3d\s*secure)/i, reason: 'Personal/financial data exposure' },
];

const EXTREME_PATTERNS: Array<{ re: RegExp; reason: string; severity: 'critical' | 'high' }> = [
  { re: /(öldür[üu]r[üu]m|seni\s*öldürece[gğ]im|kanını\s*döke[cç]e[gğ]im|canını\s*alaca[gğ]ım|katledece[gğ]im)/i, reason: 'Death threat', severity: 'critical' },
  { re: /(isis|iq|daeş|daesh|pkk|fetö|terör\s*(örgütü)?\s*(yaşasın|destekliyorum|savaşçıları))/i, reason: 'Terrorism propaganda', severity: 'critical' },
  { re: /(çocuk\s*(pornosu|porno|porno\s*video|seks)|küçük\s*(seks|cinsel)|pedofil)/i, reason: 'CSAM', severity: 'critical' },
  { re: /(islam|müslüman|kuran|kur'an|quran|muhammed|peygamber|allah|tanrı|rab)\S{0,3}\s{0,3}(sik|orospu|amk|am[ıi]na|piç|ibne|gavat|yarak|yarrak|taşak|göt)/i, reason: 'Severe religious insult', severity: 'high' },
  { re: /(sik|orospu|amk|am[ıi]na|piç|ibne|gavat|yarak|yarrak|taşak|göt)\S{0,3}\s{0,3}(islam|müslüman|kuran|kur'an|quran|muhammed|peygamber|allah|tanrı)/i, reason: 'Severe religious insult', severity: 'high' },
  { re: /(allah|tanrı|muhammed|peygamber|islam|kuran|kur'an).{0,20}(lanet|kahretsin|belası|belasını|cezasını|yok\s*olsun|gebertsin)/i, reason: 'Religious insult', severity: 'high' },
  { re: /(hristiyanlık|yahudilik|musevilik|budizm|hinduizm|incil|tevrat|kilise|sinagog|havra|cami)\S{0,3}\s{0,3}(sik|orospu|amk|am[ıi]na|piç|ibne|gavat|yarak|yarrak)/i, reason: 'Severe religious insult', severity: 'high' },
  { re: /(sik|orospu|amk|am[ıi]na|piç|ibne|gavat|yarak|yarrak)\S{0,3}\s{0,3}(hristiyanlık|yahudilik|musevilik|budizm|hinduizm|incil|tevrat|kilise|sinagog|havra|cami)/i, reason: 'Severe religious insult', severity: 'high' },
];
...
function localProfanityHeuristic(text: string): { hit: boolean; reason: string } {
  for (const re of TARGETED_PROFANITY) {
    if (re.test(text)) return { hit: true, reason: 'Targeted profanity/harassment' };
  }
...
function localSpamHeuristic(text: string): { hit: boolean; reason: string } | null {
  ...
  if (linkMatches.length >= 3) return { hit: true, reason: 'Spam: too many links (>2)' };
...
function localProfileSpamHeuristic(text: string): { hit: boolean; reason: string } {
  const PROFILE_SPAM: Array<{ re: RegExp; reason: string }> = [
    { re: /(\b\d[\d.,]*\s?(?:tl|₺|usd|\$|euro?|€)\b).{0,60}(hemen|tıkla|kazan|kaydol|ücretsiz|bedava)/i, reason: 'Spam: money + call to action' },
    { re: /(garanti kazanç|%100 kazanç|günde \d+ tl|anında para|hızlı zengin|mlm|piramit|ponzi)/i, reason: 'Spam: scam/fraud' },
    { re: /(takipçi\s*(kazan|sat|al|artır)|abone\s*(kazan|sat|al)|like\s*(kazan|sat))/i, reason: 'Spam: fake engagement' },
    { re: /(bedava para|kolay para|günde \d+ tl kazan|yatırımsız|risksiz kazanç)/i, reason: 'Spam: money promise' },
    { re: /(iban|banka\s*hesap|kart\s*bilgi|kredi\s*kart\s*no|cvv\b|3d\s*secure)/i, reason: 'Personal/financial data exposure' },
  ];
  for (const { re, reason } of PROFILE_SPAM) {
    if (re.test(text)) return { hit: true, reason };
  }
  return { hit: false, reason: '' };
}

function localGamblingHeuristic(text: string): { hit: boolean; reason: string } {
  const GAMBLING_PATTERNS: Array<{ re: RegExp; reason: string }> = [
    { re: /(bahis|casino|kumar|slot|bet|1xbet|bwin|betboo|superbahis|youwin|mobilbahis)/i, reason: 'Gambling content' },
    { re: /(tipobet|rivalo|betnano|canlı\s*bahis|bahis\s*sitesi|bahis\s*oranı)/i, reason: 'Gambling site' },
  ];
  for (const { re, reason } of GAMBLING_PATTERNS) {
    if (re.test(text)) return { hit: true, reason };
  }
  return { hit: false, reason: '' };
}
...
function localFallback(text: string): ModerationResult {
  ...
  if (pol.hit) return { safe: false, severity: 'medium', category: 'politics', reason: 'Political content', confidence: 0.7 };
...
export async function checkTextContent(
  ...
  const isProfile = meta?.contentType === 'profile';
  ...
  // Platform yönlendirme, anlamsız metin → atlanır
  if (isProfile) {
    const prof = localProfanityHeuristic(combined);
    if (prof.hit) {
      return { safe: false, severity: 'high', category: 'profanity', reason: prof.reason, confidence: 0.8 };
    }
    const gambling = localGamblingHeuristic(combined);
    if (gambling.hit) {
      return { safe: false, severity: 'medium', category: 'gambling', reason: gambling.reason, confidence: 0.85 };
    }
    const politics = localPoliticsHeuristic(combined);
    if (politics.hit) {
      return { safe: false, severity: 'medium', category: 'politics', reason: 'Siyasi içerik', confidence: 0.7 };
    }
    const profileSpam = localProfileSpamHeuristic(combined);
    if (profileSpam.hit) {
      return { safe: false, severity: 'medium', category: 'spam_scam', reason: profileSpam.reason, confidence: 0.8 };
    }
    try {
...
export async function evaluateUserReports(
  ...
  try {
    ...
    if (json.action === 'moderate') {
      return {
        shouldModerate: true,
        reason: (json.reason as string) || `${totalReportCount} reports: AI flagged for moderation`,
    ...
    return {
      ...
      reason: (json.reason as string) || `${totalReportCount} reports reviewed: No violation found`,
      severity: null,
      confidence: typeof json.confidence === 'number' ? json.confidence : 0.5,
    };
  } catch {
    ...
    return {
      shouldModerate: true,
      reason: 'AI evaluation error, human moderation required',
      severity: 'low',
      confidence: 0,
    };
  }
...
// ============================================================
// AUTO ACCOUNT MODERATION — 5+ flags within 10min → place account under moderation
// ============================================================

export async function checkAutoAccountModeration(admin: SupabaseClient, userId: string): Promise<void> {
  try {
    // Hesap zaten moderasyonda/kapalı/silinmişse tekrar alma
    ...
    // Kullanıcının son 10dk'da flag alan postlarını bul
    const { data: recentPosts } = await admin
      .from('posts')
      .select('id')
      .eq('author_id', userId)
      .eq('is_nsfw', true)
      .gte('moderation_due_at', tenMinAgo)
      .limit(5);

    if (!recentPosts || recentPosts.length < 5) return;

    const tErrors = await getTranslations("apiErrors");

    await admin.from('profiles').update({
      status: 'moderation',
      moderation_reason: tErrors('autoFlagged5Plus'),
    }).eq('user_id', userId);
    ...
    await admin.from('moderation_decisions').insert({
      ...
      target_id: userId,
      decision: 'flagged',
      reason: tErrors('autoFlagged5PlusAccount'),
      moderator_id: 'system',
    });
  } catch {}
}
