...
/**
 * Redirect URL güvenlik kontrolü — sadece aynı origin'e ait path'lere izin verir.
 * Dış sitelere yönlendirme (open redirect) saldırılarını engeller.
 */
export function isSafeRedirectUrl(url: string | null | undefined): boolean {
  ...
  if (url.includes("%2f") || url.includes("%2F") || url.includes("%00")) return false;
  try {
