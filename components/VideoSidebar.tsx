...
export function VideoSidebarSkeleton({ count = 4, title, compact }: { count?: number; title?: string; compact?: boolean }) {
      ...
      <div className="space-y-1.5">
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} className="flex gap-2.5 p-1.5 -mx-1.5">
            <div className={`${thumbW} rounded-md bg-bg-secondary animate-pulse shrink-0`} />
            <div className="flex-1 min-w-0 py-0.5 space-y-2">
              ...
              <div className="h-[9px] w-[75%] bg-bg-secondary rounded-[5px] animate-pulse mt-1" />
            </div>
          </div>
        ))}
      </div>
