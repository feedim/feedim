...
export default function VideoPlayerClient({
  ...
  useEffect(() => {
    ...
    // Mute video original audio completely, play external sound at full volume
    video.muted = true;
    video.volume = 0;
    audio.loop = true;

    const onPlay = () => { audio.currentTime = video.currentTime % (audio.duration || Infinity); audio.play().catch(() => {}); };
    const onPause = () => audio.pause();
    const onSeeked = () => { audio.currentTime = video.currentTime % (audio.duration || Infinity); };
    // Intercept mute toggle: keep video muted, redirect mute to external audio
    const onVolumeChange = () => {
      if (!video.muted) {
        // User tried to unmute video — keep video muted, unmute audio instead
        video.muted = true;
        video.volume = 0;
        audio.muted = false;
        audio.volume = 1;
      } else {
        // User muted — mute audio too
        audio.muted = true;
      }
    };
  ...
  return (
    <div className="relative sm:rounded-lg sm:overflow-hidden">
      ...
      {!postRollActive && (
        ...
        >
          <span className="text-white text-[0.72rem] font-medium">{t("autoplay")}</span>
          <button
      ...
      {ended && countdown !== null && countdown > 0 && (
        <div className="absolute inset-0 bg-black/90 flex items-center justify-center z-20 overflow-hidden pb-2">
          <div className="text-center w-full max-w-[220px] px-3">
            ...
            <p className="text-white/50 text-[0.7rem] mb-0.5">{t("nextVideo")}</p>
            {nextVideoTitle && (
      ...
      {ended && countdown === null && (
        <div className="absolute inset-0 bg-black/95 flex flex-col items-center justify-center z-20 px-4 pb-2">
          ...
          <p className="text-white/50 text-[0.72rem]">{t("replay")}</p>
        </div>
      )}
    </div>
  );
