  ...
  const resolvedSubtitle = subtitle
    ...
    : autoSubtitle
      ? user.mutual_count && user.mutual_count > 0
        ? t("mutualFollowCount", { count: formatCount(user.mutual_count, locale) })
        : user.bio || undefined
      : undefined;
  ...
  return (
    <div className="flex items-center gap-2 py-2 px-2 rounded-[14px] hover:bg-bg-secondary transition">
      ...
      <Link href={`/u/${user.username}`} onClick={onNavigate} className="flex-1 min-w-0">
        ...
        {resolvedSubtitle && (
          <p className="text-[0.66rem] text-text-muted mt-0.5 truncate max-w-[200px]">{resolvedSubtitle.length > 50 ? resolvedSubtitle.slice(0, 50) + "…" : resolvedSubtitle}</p>
        )}
      </Link>
