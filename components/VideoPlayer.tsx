...
type ScreenOrientationControls = {
...
function getOrientationControls(): ScreenOrientationControls | null {
  if (typeof screen === "undefined") return null;
  return screen.orientation as ScreenOrientationControls | null;
}

async function lockLandscapeOrientation() {
  try {
    await getOrientationControls()?.lock?.("landscape");
  } catch {}
}

function unlockOrientation() {
  try {
    getOrientationControls()?.unlock?.();
  } catch {}
}
...
const VideoPlayerInner = forwardRef<HTMLVideoElement, VideoPlayerProps>(function VideoPlayer({ src, hlsUrl, poster, onEnded, autoStart, disabled, moment, externalMuted, externalPaused, loop, videoClassName, preloadHint, onNext, onPrev, hasNext, hasPrev, autoplayNext, onToggleAutoplay }, fwdRef) {
  ...
  const toggleFullscreen = useCallback(async () => {
    ...
    if (!c) return;
    const fsDocument = getFullscreenDocument();

    // Exit real fullscreen if active
    if (document.fullscreenElement || fsDocument.webkitFullscreenElement) {
      unlockOrientation();
      if (document.exitFullscreen) await document.exitFullscreen();
      else await fsDocument.webkitExitFullscreen?.();
      return;
    }
    ...
    const fsEnabled = document.fullscreenEnabled || fsDocument.webkitFullscreenEnabled;
    ...
    if (fsEnabled) {
      try {
        ...
        const webkitContainer = c as WebkitFullscreenElement;
  ...
  useEffect(() => {
    ...
    setFullscreen(fakeFullscreen || !!document.fullscreenElement || !!getFullscreenDocument().webkitFullscreenElement);
  }, [fakeFullscreen]);
  ...
  useEffect(() => {
    ...
    const onFsChange = () => {
      const isFs = !!document.fullscreenElement || !!getFullscreenDocument().webkitFullscreenElement;
  ...
  useEffect(() => {
    ...
    const handle = (e: KeyboardEvent) => {
      ...
      // Use getBoundingClientRect instead of offsetParent — offsetParent is null for position:fixed (fullscreen/cinema)
      const rect = c.getBoundingClientRect();
      if (rect.width === 0 || rect.height === 0) return;
      const v = videoRef.current;
      if (!v) return;
      // Skip modifier combos (let browser/GlobalHotkeys handle Cmd+/, etc.)
      if (e.metaKey || e.ctrlKey || e.altKey) {
        // Only intercept Ctrl/Cmd+S and Ctrl/Cmd+U (block save/view-source)
        if ((e.ctrlKey || e.metaKey) && (e.key === "s" || e.key === "u")) { e.preventDefault(); e.stopImmediatePropagation(); }
        return;
      }
      // Escape exits fake fullscreen
      if (e.key === "Escape" && fakeFullscreen) { unlockOrientation(); setFakeFullscreen(false); e.preventDefault(); e.stopImmediatePropagation(); return; }
      const dur = v.duration || 0;
      switch (e.key) {
        case " ": case "k": e.preventDefault(); e.stopImmediatePropagation(); togglePlay(); break;
        case "ArrowLeft": e.preventDefault(); e.stopImmediatePropagation(); v.currentTime = Math.max(0, v.currentTime - 5); updateProgress(); showControls(); break;
        case "ArrowRight": e.preventDefault(); e.stopImmediatePropagation(); v.currentTime = Math.min(dur, v.currentTime + 5); updateProgress(); showControls(); break;
        case "j": e.preventDefault(); e.stopImmediatePropagation(); v.currentTime = Math.max(0, v.currentTime - 10); updateProgress(); showControls(); break;
        case "l": e.preventDefault(); e.stopImmediatePropagation(); v.currentTime = Math.min(dur, v.currentTime + 10); updateProgress(); showControls(); break;
        case "ArrowUp": e.preventDefault(); e.stopImmediatePropagation(); changeVolume(Math.min(1, v.volume + 0.05)); showControls(); break;
        case "ArrowDown": e.preventDefault(); e.stopImmediatePropagation(); changeVolume(Math.max(0, v.volume - 0.05)); showControls(); break;
        case "f": e.preventDefault(); e.stopImmediatePropagation(); toggleFullscreen(); break;
        case "m": e.preventDefault(); e.stopImmediatePropagation(); toggleMute(); break;
        case "t": e.preventDefault(); e.stopImmediatePropagation(); toggleCinema(); break;
        case "p": e.preventDefault(); e.stopImmediatePropagation(); togglePiP(); break;
        case "Home": e.preventDefault(); e.stopImmediatePropagation(); v.currentTime = 0; updateProgress(); showControls(); break;
        case "End": e.preventDefault(); e.stopImmediatePropagation(); v.currentTime = dur; updateProgress(); showControls(); break;
        case "0": case "1": case "2": case "3": case "4":
        case "5": case "6": case "7": case "8": case "9":
          e.preventDefault(); e.stopImmediatePropagation(); v.currentTime = dur * (Number(e.key) / 10); updateProgress(); showControls(); break;
        case ">": e.preventDefault(); e.stopImmediatePropagation(); changeSpeed(Math.min(2, (v.playbackRate || 1) + 0.25)); break;
        case "<": e.preventDefault(); e.stopImmediatePropagation(); changeSpeed(Math.max(0.25, (v.playbackRate || 1) - 0.25)); break;
        default: return; // Don't block unhandled keys (g, c, b, s, etc.) — let GlobalHotkeys handle them
      }
    };
  ...
  return (
    ...
    <div ref={wrapperRef} className="relative overflow-visible">
      ...
      >
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent pointer-events-none sm:rounded-b-lg" />
          ...
          <div className="relative mx-auto max-w-[calc(100%-32px)] pb-2.5 pt-10">
            ...
            {/* Buttons row */}
              <div className="flex items-center gap-2 text-white">
                ...
                {/* Volume */}
                <div className="flex items-center gap-[2px] group/vol">
