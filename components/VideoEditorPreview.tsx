...
export default function VideoEditorPreview({
  ...
  return (
    ...
    >
      ...
      {showScrubber && !uploading && src && duration > 0 && (
        <div
          ref={scrubberRef}
      ...
      {uploading && (
        <div className="absolute inset-0 bg-black/80 backdrop-blur-sm rounded-xl flex flex-col items-center justify-center z-10">
          ...
          <button onClick={(e) => { e.stopPropagation(); onCancelUpload(); }} className="text-xs text-error font-semibold hover:underline mt-3">
            {t("cancelUpload")}
          </button>
        </div>
      )}
