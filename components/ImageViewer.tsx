...
export default function ImageViewer({ images, initialIndex, open, onClose }: ImageViewerProps) {
  ...
  if (!open || !hydrated || images.length === 0) return null;
  ...
  return createPortal(
    ...
    >
      ...
      {multipleImages && currentIndex > 0 && (
        <button className="imgsp-nav imgsp-prev" onClick={goPrev} aria-label={t("previous")}>
      ...
      {multipleImages && currentIndex < images.length - 1 && (
        <button className="imgsp-nav imgsp-next" onClick={goNext} aria-label={t("next")}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
