...
export default function RemovedPostTemplate({ reason, decisionCode }: RemovedPostTemplateProps) {
  ...
  return (
    <div className="px-4 sm:px-5 py-8 max-w-lg mx-auto text-center">
      <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-2xl p-6 mb-6">
        ...
        <h2 className="text-lg font-bold text-red-800 dark:text-red-300 mb-2">{t("removedTitle")}</h2>
        <p className="text-sm text-red-700 dark:text-red-400 mb-4">
          {t("removedDescription")}
        </p>
        {reason && (
          <p className="text-xs text-red-600 dark:text-red-500 mb-2">
            {t("removedReasonLabel")}: {reason}
          </p>
        )}
        {decisionCode && (
          <p className="text-xs text-red-600 dark:text-red-500 mb-4">
            {t("removedDecisionLabel")}: <strong>#{decisionCode}</strong>
          </p>
        )}
        {decisionCode && (
          <NewTabLink
