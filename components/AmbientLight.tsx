...
interface AmbientPerfProfile {
  isConstrained: boolean;
  scanDelayMs: number;
  videoSampleStride: number;
  videoThemeSyncEvery: number;
  videoPollMs: number;
}
...
function detectAmbientPerfProfile(): AmbientPerfProfile {
  if (typeof window === "undefined") {
    return {
      isConstrained: false,
      scanDelayMs: 200,
      videoSampleStride: 30,
      videoThemeSyncEvery: 5,
      videoPollMs: 150,
    };
  }
  ...
  const isConstrained = coarsePointer || saveData || slowNetwork || lowMemory || lowCpu;

  return {
    isConstrained,
    scanDelayMs: isConstrained ? 360 : 200,
    videoSampleStride: isConstrained ? 60 : 30,
    videoThemeSyncEvery: isConstrained ? 12 : 5,
    videoPollMs: isConstrained ? 600 : 150,
  };
}
...
function sampleThemeColor(canvas: HTMLCanvasElement, theme: string): string | null {
  try {
    ...
    if (theme === "dark") {
      ...
      // Light: mix ~22% into #ffffff (stronger tint so glow is visible against bg-secondary)
      r = Math.round(255 - (255 - r) * 0.22);
      g = Math.round(255 - (255 - g) * 0.22);
      b = Math.round(255 - (255 - b) * 0.22);
    }
...
export default function AmbientLight({ imageSrc, videoMode }: AmbientLightProps) {
  ...
  const perfProfileRef = useRef<AmbientPerfProfile>(detectAmbientPerfProfile());
  ...
  useEffect(() => {
    if (localStorage.getItem("fdm-ambient-light") !== "on") return;
    setMounted(true);
  ...
  useEffect(() => {
    const handler = () => {
      const isOn = localStorage.getItem("fdm-ambient-light") === "on";
      if (isOn) {
        setMounted(true);
        document.documentElement.dataset.ambientLight = "1";
      } else {
  ...
  useEffect(() => {
    if (!mounted || imageSrc || autoSrc) return;
    ...
    const scheduleScan = () => {
      ...
      const run = () => {
        scanQueued = false;
        if (findImage()) observer?.disconnect();
      };
      if (idleWindow.requestIdleCallback) {
        const idleId = idleWindow.requestIdleCallback(run, {
          timeout: perfProfileRef.current.scanDelayMs,
        });
        cancelScheduledScan = () => {
      ...
      const timer = window.setTimeout(run, perfProfileRef.current.scanDelayMs);
      cancelScheduledScan = () => window.clearTimeout(timer);
    };
  ...
  }, [mounted, imageSrc, pathname, autoSrc]);
    ...
    try {
      ...
      if (shouldFade) {
        ...
        requestAnimationFrame(() => {
          ...
          if (++videoThemeRef.current % perfProfileRef.current.videoThemeSyncEvery === 0) syncThemeColor();
          requestAnimationFrame(() => {
            canvas.style.opacity = String(TARGET_OPACITY);
          });
        });
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (++frameCount % perfProfileRef.current.videoSampleStride === 0) drawVideoFrame();
    ...
    }, perfProfileRef.current.videoPollMs);
