...
export default function PostStatsModal({ open, onClose, postId }: PostStatsModalProps) {
  ...
  const statsUrl = withCacheScope(`/api/posts/${postId}/stats?tz=${tz}`, `stats:${locale}`);

  useLayoutEffect(() => {
    if (!open) return;
    const cached = readCache(statsUrl) as {
      post?: { title?: string; content_type?: string };
      totals?: Totals;
      readStats?: ReadStats;
      videoStats?: VideoStatsData | null;
      viewsByDay?: DayData[];
      recentComments?: CommentData[];
    } | null;
    if (!cached) return;
    setIsVideoOrMoment(cached.post?.content_type === "video" || cached.post?.content_type === "moment");
    setTotals(cached.totals || { views: 0, likes: 0, comments: 0, saves: 0, shares: 0 });
    setReadStats(cached.readStats || { avgReadDuration: 0, avgReadPercentage: 0, qualifiedReads: 0 });
    setVideoStats(cached.videoStats || null);
  ...
  const loadStats = useCallback(async () => {
    if (!readCache(statsUrl)) setLoading(true);
    try {
      const data = await fetchWithCache(statsUrl, { ttlSeconds: FRESHNESS_WINDOWS.postStats }) as {
        post?: { title?: string; content_type?: string };
        totals?: Totals;
        readStats?: ReadStats;
        videoStats?: VideoStatsData | null;
...
/* ── StatBox ── */
function StatBox({ icon: Icon, label, value }: { icon: ComponentType<{ className?: string }>; label: string; value: number }) {
  return (
