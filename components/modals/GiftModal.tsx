...
export default function GiftModal({ open, onClose, postId, onGiftSent }: GiftModalProps) {
  ...
  useEffect(() => {
    if (open) {
      setSelected(null);
      setCount(1);
      if (!giftsLoadedRef.current) {
        giftsLoadedRef.current = true;
        loadData();
      } else {
        // Only refresh balance on subsequent opens
        fetch("/api/profile").then(r => r.json()).then(d => setBalance(d.profile?.coin_balance || 0)).catch(() => {});
      }
    }
  }, [open, loadData]);
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pt-3 pb-3">
          ...
          <div className="flex items-center gap-1.5 bg-bg-tertiary backdrop-blur-sm px-3 py-1.5 rounded-full">
            ...
            {balance !== null ? (
              <span className="text-[0.78rem] font-bold">{formatCount(balance)}</span>
            ) : (
              <div className="h-[10px] w-6 bg-bg-secondary rounded-[5px] animate-pulse" />
            )}
          </div>
        </div>
            ...
            {giftKeys.map(key => {
              ...
              return (
                <button
                  ...
                  disabled={!canAfford}
                  className={`relative flex items-center gap-2 px-2.5 py-2.5 rounded-lg transition-all duration-200 ${
                    isSelected
                ...
                >
                  <div className={`shrink-0 w-7 h-7 flex items-center justify-center transition-transform duration-200 ${isSelected ? "scale-110" : ""}`}>
                    {GIFT_ICONS[key](28)}
                  </div>
                  <div className="flex flex-col items-start min-w-0">
                    <span className="text-[0.74rem] font-semibold leading-tight truncate">{tg(gift.labelKey.replace("gifts.", ""))}</span>
                    <span className="text-[0.64rem] text-text-muted flex items-center gap-0.5 mt-0.5">
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center pointer-events-auto" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-white font-bold text-[1.15rem] gift-celebration-text" style={{ animationDelay: "0.4s" }}>
              {tg(GIFT_TYPES[celebration.key].labelKey.replace("gifts.", ""))}{celebration.count > 1 ? ` x${celebration.count}` : ""}
            </p>
