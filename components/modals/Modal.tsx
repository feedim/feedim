...
// Global modal stack — ESC only closes the topmost modal
...
function isModalDragBlockedTarget(target: EventTarget | null, popup: HTMLElement): boolean {
  let el = target as HTMLElement | null;
  while (el && el !== popup) {
    if (el.nodeType !== 1) {
      el = el.parentElement;
    ...
    if (el.hasAttribute("data-modal-no-drag")) return true;

    const tag = el.tagName.toLowerCase();
    if (["input", "textarea", "select", "button", "a", "label", "video", "audio", "iframe", "summary"].includes(tag)) {
      return true;
    }

    const role = el.getAttribute("role");
    if (role && ["button", "link", "slider", "textbox", "combobox", "switch", "tab", "checkbox", "radio", "menuitem", "option"].includes(role)) {
      return true;
    }
    ...
    if (
      ...
      className.includes("cursor-grabbing") ||
      className.includes("touch-none")
    ) {
      return true;
    }
...
export default function Modal({
  ...
  const shouldCenter = centerOnMobile || (centerOnDesktop && isDesktop);
  ...
  const isBottomSheet = resolvedType === 1 && !shouldCenter;
  ...
  // Register/unregister in global modal stack
  useEffect(() => {
    if (open) {
      modalIdRef.current = ++modalIdCounter;
      openModalIds.add(modalIdRef.current);
    } else {
      openModalIds.delete(modalIdRef.current);
    }
    return () => { openModalIds.delete(modalIdRef.current); };
  }, [open]);
  ...
  useEffect(() => {
    ...
    const handleEsc = (e: KeyboardEvent) => {
      ...
      // Only close if this is the topmost (most recently opened) modal
      const topId = Math.max(...openModalIds);
      if (modalIdRef.current === topId) handleClose();
    };
  ...
  const startDrag = useCallback((clientY: number, clientX: number, target: EventTarget | null, force = false) => {
    const sheet = sheetRef.current;
    if (!sheet) return;
    if (!force && isModalDragBlockedTarget(target, sheet)) return false;
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent) => {
    startDrag(e.touches[0].clientY, e.touches[0].clientX, e.target, true);
  }, [startDrag]);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    e.preventDefault();
    if (!startDrag(e.clientY, e.clientX, e.target, true)) return;
  ...
  const sheetLayoutClasses = resolvedType === 2
    ...
    : `relative w-full ${sizeClasses[size]} bg-bg-secondary ${centerOnMobile ? "!rounded-[20px] max-h-[90dvh]" : "rounded-t-[20px] max-h-[90dvh]"} ${centerOnDesktop ? "sm:!rounded-[20px] sm:!max-h-[85vh]" : "sm:max-h-[95dvh]"} ${fullHeight ? "min-h-[90dvh] sm:min-h-[95dvh]" : ""} overflow-hidden flex flex-col will-change-transform`;

  const containerAlignClasses = resolvedType === 2
    ? `fixed inset-0 ${zIndex} flex items-stretch justify-end`
    : `fixed inset-0 ${zIndex} flex ${centerOnMobile ? "items-center px-3" : "items-end"} ${centerOnDesktop ? "sm:items-center sm:px-0" : ""} justify-center`;
