...
const planHeroTitles: Record<string, string> = {
  basic: "Feedim Super",
  pro: "Feedim Pro",
  max: "Feedim Max",
  business: "Feedim Business",
};

const planSubtitleKeys: Record<string, string> = {
  basic: "planSubtitleStarter",
  pro: "planSubtitlePro",
  max: "planSubtitleMax",
  business: "planSubtitleBusiness",
};

interface Feature {
  key: string;
  dByTier?: Partial<Record<"basic" | "pro" | "max" | "business", string>>;
  basic: boolean;
  pro: boolean;
  max: boolean;
  business: boolean;
...
const allFeatures: Feature[] = [
  { key: "AdFree", basic: true, pro: true, max: true, business: true },
  { key: "MoreFollows", dByTier: { basic: "1.25x", pro: "2.2x", max: "2.5x", business: "2.5x" }, basic: true, pro: true, max: true, business: true },
  { key: "MoreLikes", dByTier: { basic: "1.25x", pro: "2.2x", max: "2.5x", business: "2.5x" }, basic: true, pro: true, max: true, business: true },
  { key: "MoreComments", dByTier: { basic: "1.3x", pro: "1.5x", max: "1.9x", business: "1.9x" }, basic: true, pro: true, max: true, business: true },
  { key: "2FA", basic: true, pro: true, max: true, business: true },
  { key: "VerifiedBadge", basic: false, pro: true, max: true, business: true },
  { key: "Monetization", basic: false, pro: true, max: true, business: true },
  { key: "HighlightedComments", basic: false, pro: true, max: true, business: true },
  { key: "Analytics", basic: false, pro: true, max: true, business: true },
  { key: "ExplorePriority", basic: false, pro: false, max: true, business: true },
  { key: "SearchPriority", basic: false, pro: false, max: false, business: true },
  { key: "SuggestionPriority", basic: false, pro: false, max: false, business: true },
  { key: "ProfileVisitors", basic: false, pro: false, max: true, business: true },
  { key: "LongComment", basic: false, pro: false, max: true, business: true },
  { key: "LongPost", basic: false, pro: false, max: true, business: true },
  { key: "MultipleLinks", basic: false, pro: false, max: true, business: true },
  { key: "EarlyAccess", basic: false, pro: false, max: true, business: true },
  { key: "PrioritySupport", basic: false, pro: false, max: false, business: true },
  { key: "FastReview", basic: false, pro: false, max: true, business: true },
  { key: "BusinessAccount", basic: false, pro: false, max: false, business: true },
  { key: "ContactInfo", basic: false, pro: false, max: false, business: true },
];
...
export default function PremiumPage() {
  ...
  const currentPeriod = t(isYearly ? "periodYear" : "periodMonth");
    ...
    <div className="min-h-screen text-text-primary">
      ...
      <header>
        <nav className="container mx-auto px-6 py-6 flex items-center justify-between">
          <Link href="/" aria-label="Feedim" className="flex items-center gap-3">
      ...
      <main className="container mx-auto px-4 sm:px-6 pt-8 pb-12 max-w-full sm:max-w-[92%]">
        ...
        <div className="text-center mb-9">
          ...
          <h2 className="text-[1.5rem] sm:text-[1.8rem] font-extrabold mb-1.5 transition-all">
            {planHeroTitles[selectedPlan] || "Feedim Premium"}
          </h2>
          <p className="text-text-muted text-[0.9rem] sm:text-[0.95rem] leading-relaxed transition-all max-w-md mx-auto">
            {planSubtitleKeys[selectedPlan] ? t(planSubtitleKeys[selectedPlan] as any) : t("defaultSubtitle")}
          </p>
        </div>
        ...
        <div className="premium-plans-grid mb-7">
          {plans.map(plan => {
            ...
            const period = t(isYearly ? "periodYear" : "periodMonth");
            ...
            return (
              ...
              >
                {isLoggedIn && userCurrentPlan === plan.id ? (
                  <span className="premium-plan-badge">{t("currentPlan")}</span>
                ) : plan.popular ? (
                  <span className="premium-plan-badge">{t("popular")}</span>
                ) : null}
                ...
                {isYearly && (
                  <p className="text-[0.78rem] md:text-[0.72rem] font-semibold text-accent-main mb-2">{monthlySaving.toLocaleString(locale)}₺ {t("saveMoney").toLowerCase()}</p>
                )}
                      ...
                      <div className="flex items-start gap-2.5 md:gap-2 text-left py-[11px] md:py-[8px]">
                        ...
                        <div className="min-w-0">
                          <p className={`font-semibold leading-snug ${isExpanded ? "text-[0.92rem] md:text-[0.82rem]" : "text-[0.88rem] md:text-[0.82rem]"}`}>
                            {t(`feature${f.key}` as any)}
                            {f.key === "VerifiedBadge" && plan.tier !== "basic" && (
                          ...
                          <p className={`text-text-muted leading-snug mt-0.5 md:mt-0 ${isExpanded ? "text-[0.82rem] md:text-[0.72rem]" : "text-[0.78rem] md:text-[0.72rem]"}`}>
                            {f.dByTier?.[plan.tier]
                              ? `${t(`feature${f.key}Desc` as any)} (${f.dByTier[plan.tier]})`
                              : t(`feature${f.key}Desc` as any)
                            }
                          </p>
                        </div>
                      </div>
                ...
                {hasMore && (
                  ...
                  >
                    {isExpanded ? t("showLess") : t("showMore", { count: features.length - PREVIEW_COUNT })}
                  </button>
                )}
              </div>
            );
          ...
          <div className="rounded-[14px] overflow-hidden border border-border-primary">
            ...
            {allFeatures.map((f, i) => (
              <div key={i} className="grid grid-cols-[1fr_repeat(4,36px)] sm:grid-cols-[1fr_50px_50px_50px_60px] items-center px-3 sm:px-4 py-3 sm:py-2.5 border-t border-border-primary">
                <span className="text-[0.8rem] sm:text-[0.82rem] font-medium pr-2">{t(`feature${f.key}` as any)}</span>
        ...
        <div className="mb-12">
          <h3 className="text-[1.1rem] sm:text-[1.1rem] font-bold mb-4">{t("faqTitle")}</h3>
            ...
            {faqKeys.map((faqKey, i) => {
              ...
              return (
                <div key={i} className="rounded-[13px] bg-bg-secondary overflow-hidden">
                  ...
                  >
                    <span className="text-[0.92rem] sm:text-[0.95rem] font-semibold flex-1 min-w-0 pr-3">{t(`faq${faqKey}Q` as any)}</span>
        ...
        <div className="rounded-2xl bg-accent-main/[0.06] p-6 sm:p-10 text-center">
          ...
          <h3 className="text-[1.1rem] sm:text-lg font-bold mb-2">{t("needHelp")}</h3>
          <p className="text-[0.88rem] sm:text-sm text-text-muted mb-6 max-w-sm mx-auto leading-relaxed">{t("needHelpDesc")}</p>
          <NewTabLink href="/help/contact" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-accent-main text-white font-semibold text-[0.88rem] sm:text-sm hover:opacity-90 transition">
            {t("contactUs")}
          </NewTabLink>
        </div>
      </main>
