...
export async function GET(
  ...
  try {
    ...
    if (error || !sound) {
      ...
      return NextResponse.json({ error: tErrors("soundNotFound") }, { status: 404 });
    }
...
export async function PATCH(
  ...
  try {
    ...
    // Fetch sound data before update for R2 cleanup
    let soundBeforeUpdate: { audio_url: string | null; cover_image_url: string | null; is_original: boolean } | null = null;
    if (status === "removed") {
      const { data: pre } = await admin.from("sounds").select("audio_url, cover_image_url, is_original").eq("id", id).single();
      soundBeforeUpdate = pre;
    }
    ...
    // R2 cleanup when sound is removed
    if (status === "removed" && soundBeforeUpdate) {
      try {
        if (soundBeforeUpdate.audio_url && !soundBeforeUpdate.is_original) {
          const k = r2KeyFromUrl(soundBeforeUpdate.audio_url);
          if (k) await deleteFromR2(k).catch(() => {});
        }
        if (soundBeforeUpdate.cover_image_url) {
          const k = r2KeyFromUrl(soundBeforeUpdate.cover_image_url);
          if (k) await deleteFromR2(k).catch(() => {});
        }
      } catch {}
    }
