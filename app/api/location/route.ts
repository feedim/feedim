...
export async function POST(req: NextRequest) {
  ...
  if (typeof latitude === "number" && typeof longitude === "number") {
    ...
    try {
      const geoRes = await fetch(
        ...
        { headers: { "User-Agent": "Feedim/1.0" }, signal: AbortSignal.timeout(5000) }
      );
      if (geoRes.ok) {
    ...
    if (clientIp && clientIp !== "127.0.0.1" && clientIp !== "::1") {
      try {
        const ipRes = await fetch(`http://ip-api.com/json/${clientIp}?fields=status,country,countryCode,regionName,city,lat,lon`, { signal: AbortSignal.timeout(5000) });
        if (ipRes.ok) {
  ...
  if (!city && !region && !countryCode) {
    return NextResponse.json({ location: null, message: tErrors("locationNotDetermined") });
  }
