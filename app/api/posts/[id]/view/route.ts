...
export async function POST(
  ...
  try {
    ...
    // Fetch post info for duration capping + private account check
    const { data: postInfo } = await admin
      .from('posts')
      .select('content_type, reading_time, video_duration, author_id, profiles!posts_author_id_fkey(account_private)')
      .eq('id', postId)
      .single();
    if (postInfo) {
      const _pa = Array.isArray(postInfo.profiles) ? postInfo.profiles[0] : postInfo.profiles;
      if ((_pa as any)?.account_private && postInfo.author_id !== user.id) {
        const { data: _f } = await admin.from('follows').select('id')
          .eq('follower_id', user.id).eq('following_id', postInfo.author_id).maybeSingle();
        if (!_f) return NextResponse.json({ recorded: false });
      }
    }
    ...
    // All content types can earn
    const earningEligibleType = true;
    ...
    if (isQualifiedRead && viewerIsPremium && earningEligibleType) {
      ...
      if (post && post.author_id !== user.id) {
        ...
        const { data: authorMeta } = await admin
          .from('profiles')
          .select('account_type, account_private, role')
          .eq('user_id', post.author_id)
          .single();

        const authorIsAdmin = authorMeta?.role === 'admin';
        ...
        try {
          const { data: monCheck } = await admin
            ...
            .eq('user_id', post.author_id)
            .single();
          authorMonetizationEnabled = authorIsAdmin || monCheck?.monetization_enabled || false;
        } catch {}

        const authorIsProfessional = authorIsAdmin || authorMeta?.account_type === 'creator' || authorMeta?.account_type === 'business';
        const authorIsPrivate = !authorIsAdmin && authorMeta?.account_private === true;

        if (!authorMonetizationEnabled || !authorIsProfessional || authorIsPrivate) {
          // Author monetization not active — skip earning
        } else if (!authorIsAdmin && (post.spam_score || 0) >= SPAM_THRESHOLDS.earningStop) {
          ...
          if (!authorIsAdmin && (authorProfile?.profile_score ?? 100) < 15) {
            ...
            if (coinsEarned > 0) {
              // Atomic coin increment via RPC to prevent race conditions
              const { data: newBalance } = await admin.rpc("increment_coin_balance", {
                p_user_id: post.author_id,
                p_amount: coinsEarned,
              });
              // Also increment total_earned atomically
              const { error: totalEarnedErr } = await admin.rpc("increment_total_earned", {
                p_user_id: post.author_id,
                p_amount: coinsEarned,
              });
              if (totalEarnedErr) {
                ...
                await admin.from('profiles')
                  .update({ total_earned: (authorProfile.total_earned || 0) + coinsEarned })
                  .eq('user_id', post.author_id);
              }
              const updated = newBalance != null ? { coin_balance: newBalance } : null;

              if (updated) {
                const { data: authorLang } = await admin.from('profiles').select('language').eq('user_id', post.author_id).single();
                const tAuthor = await getTranslations({ locale: authorLang?.language || 'en', namespace: 'notifications' });
                await Promise.all([
                  admin.from('coin_transactions').insert({
                    ...
                    description: tAuthor("premiumReaderEarning", { multiplier: coinsEarned > 1 ? coinsEarned : 1 }),
                  }),
    ...
    if (videoAnalytics && typeof videoAnalytics === 'object') {
      ...
      try {
        ...
        logServerError('[view] analytics_events insert failed', err, { event: 'video_watch' });
      }
    }
    ...
    if (MILESTONES.includes(newViewCount) && postForMilestone) {
      ...
      await createNotification({
        ...
        object_type: 'post',
        object_id: postId,
        content: tNotif("viewMilestone", { milestone: milestoneLabel }),
      });
    }
