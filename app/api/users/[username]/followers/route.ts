...
import { countActiveFollowers } from "@/lib/followCounts";
...
export async function GET(
  ...
  try {
    ...
    const { data: profile } = await admin
      .from("profiles")
      .select("user_id, account_private, follower_count")
    ...
    // Private account: restrict follower list to owner, followers, and staff
    ...
    if (!follows || follows.length === 0) {
      // Auto-heal: if stored count > 0 but no follows found, reset
      if (page === 1 && profile.follower_count > 0) {
        void admin.from("profiles").update({ follower_count: 0 }).eq("user_id", profile.user_id);
      }
      return NextResponse.json({ users: [], hasMore: false, totalCount: 0 });
    }

    const rawHasMore = follows.length > limit;
    let ids = follows.slice(0, limit).map(f => f.follower_id);
    ...
    if (page === 1) {
      const activeCount = await countActiveFollowers(admin, profile.user_id);
      totalCount = activeCount;
      if (activeCount !== profile.follower_count) {
        void admin.from("profiles").update({ follower_count: activeCount }).eq("user_id", profile.user_id);
      }
    }
