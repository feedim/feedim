...
export async function POST(
  ...
  if (target.user_id === user.id) return NextResponse.json({ error: tErrors("cannotBlockSelf") }, { status: 400 });
  if (target.role === "admin" || target.role === "moderator") {
    return NextResponse.json({ error: tErrors("cannotBlockUser") }, { status: 403 });
  }
