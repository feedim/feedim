...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!code || typeof code !== "string") {
      return NextResponse.json({ error: tErrors("couponCodeRequired") }, { status: 400 });
    }
    ...
    if (cleanCode.length < 3 || cleanCode.length > 9) {
      return NextResponse.json({ error: tErrors("couponCodeFormat") }, { status: 400 });
    }
    if (!discountPercent || discountPercent < 1 || discountPercent > 100) {
      return NextResponse.json({ error: tErrors("discountRange") }, { status: 400 });
    }
    if (maxUses !== null && maxUses !== undefined && maxUses < 1) {
      return NextResponse.json({ error: tErrors("maxUsesMin") }, { status: 400 });
    }
    ...
    if (existing) {
      return NextResponse.json({ error: tErrors("couponCodeExists") }, { status: 400 });
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!couponId) {
      return NextResponse.json({ error: tErrors("idRequired") }, { status: 400 });
    }
