...
export async function GET(req: NextRequest) {
  try {
    ...
    if (postIds.length === 0) {
      ...
      response.headers.set("Cache-Control", buildPrivateCacheControl(FRESHNESS_WINDOWS.analyticsOverview));
      return response;
    }
    ...
    const [
      ...
      admin.from("profile_visits").select("id", { count: "exact", head: true }).eq("visited_id", user.id).gte("created_at", startDate),
    ]);
    ...
    const [
      ...
      admin.from("profile_visits").select("id", { count: "exact", head: true }).eq("visited_id", user.id).gte("created_at", prevStartDate).lt("created_at", startDate),
    ]);
    ...
    if (followerCount > 0) {
      ...
      if (followerIds.length > 0) {
        ...
        const ageRanges: Record<string, number> = { "15-17": 0, "18-24": 0, "25-34": 0, "35-44": 0, "45+": 0 };
        let ageUnknown = 0;
    ...
    const response = NextResponse.json({
      ...
      periodCounts: {
        ...
        profileVisits: periodProfileVisits || 0,
      },
      prev: {
        ...
        profileVisits: prevProfileVisits || 0,
      },
      viewsByDay, likesByDay, commentsByDay, followersByDay,
