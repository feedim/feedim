...
export async function GET(req: NextRequest) {
  ...
  if (!url) {
    return NextResponse.json({ error: tErrors("urlParameterRequired") }, { status: 400 });
  }

  if (format !== "json") {
    return NextResponse.json({ error: tErrors("jsonFormatOnly") }, { status: 501 });
  }

  // Extract slug from URL pattern: /{slug} or /post/{slug} (legacy)
  const match = url.match(/\/post\/([^/?#]+)/) || url.match(/feedim\.com\/([^/?#]+)/);
  if (!match) {
    return NextResponse.json({ error: tErrors("invalidUrl") }, { status: 404 });
  }
  ...
  if (error || !post) {
    return NextResponse.json({ error: tErrors("notFound") }, { status: 404 });
  }
