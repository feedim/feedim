...
export default function ResetPasswordPage() {
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await waitMin();
      feedimAlert("error", translateError(message, tErrors) || t("resetPasswordFailed"));
    } finally {
      setLoading(false);
    }
  };
    ...
    <AuthLayout title={t("resetPasswordTitle")} subtitle={t("resetPasswordSubtitle")}>
      <form onSubmit={handleReset} className="space-y-4">
        ...
        <div className="text-center">
          <Link href="/login" className="text-sm text-text-muted hover:text-text-primary transition hover:underline font-semibold">
            {t("backToLogin")}
          </Link>
        </div>
      </form>
