...
export default function AdminCouponsPage() {
  ...
  const handleCreateCoupon = async () => {
    if (!couponForm.code.trim()) {
      feedimAlert("error", t("codeRequired"));
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        feedimAlert("error",data.error || t("creationError"));
        return;
      }
      feedimAlert("success",`${t("creationSuccess")}: ${data.coupon.code}`);
      setCoupons([data.coupon, ...coupons]);
      setCouponForm({ code: '', discountPercent: 15, maxUses: 100, expiryHours: 720, isFree: false });
    } catch {
      feedimAlert("error",tc("tryAgainLater"));
    } finally {
  ...
  const handleDeleteCoupon = async (couponId: string) => {
    try {
      ...
      if (!res.ok) {
        feedimAlert("error",t("deletionError"));
        return;
      }
      setCoupons(coupons.filter(c => c.id !== couponId));
      feedimAlert("success",t("deletionSuccess"));
    } catch {
      feedimAlert("error",tc("tryAgainLater"));
    }
  };
  ...
  return (
    <div className="min-h-screen text-text-primary">
      <header className="z-50 min-h-[73px]">
        ...
        {loading ? (
          ...
          <div className="rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold">{t("pageTitle")}</h3>
            </div>
            ...
            <div className="space-y-3 mb-5">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-text-muted mb-1">{t("codeLabel")}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs text-text-muted mb-1">{t("discountLabel")}</label>
                  <input
                    type="number"
              ...
              <label className="flex items-center gap-2 cursor-pointer select-none">
                ...
                />
                <span className="text-sm text-text-muted">{t("freeCheckbox")}</span>
              </label>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-text-muted mb-1">{t("maxUsesLabel")}</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs text-text-muted mb-1">{t("durationLabel")}</label>
                  <input
                    type="number"
              ...
              >
                ...
                {couponCreating ? t("creating") : t("createButton")}
              </button>
            </div>
            ...
            {generalCoupons.length > 0 && (
              <div className="space-y-2">
                <p className="text-xs text-text-muted font-medium uppercase tracking-wider">{t("existingCoupons")}</p>
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs bg-accent-main/20 text-accent-main px-2 py-0.5 rounded-full">
                          {coupon.discount_percent === 100 ? t("freeTag") : `%${coupon.discount_percent}`}
                        </span>
                      </div>
                      <p className="text-xs text-text-muted mt-1">
                        {coupon.current_uses}/{coupon.max_uses || '∞'} {t("usageLabel")}
                        {coupon.expires_at && ` ${new Date(coupon.expires_at) > new Date() ? `${Math.ceil((new Date(coupon.expires_at).getTime() - Date.now()) / (1000 * 60 * 60))}${t("hoursRemaining")}` : t("expired")}`}
                      </p>
                    </div>
                    <button onClick={() => handleDeleteCoupon(coupon.id)} className="p-2 text-text-muted hover:text-error transition shrink-0" title={tc("delete")}>
                      <X className="h-4 w-4" />
                    </button>
                  </div>
                ))}
                {visibleCoupons < generalCoupons.length && (
                  ...
                  >
                    {t("showMore")} ({generalCoupons.length - visibleCoupons})
                  </button>
                )}
              </div>
            )}
