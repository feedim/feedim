      ...
      <div className="py-4 px-3 sm:px-4 max-w-xl mx-auto">
        {loading ? (
          ...
          <div className="space-y-5">
            ...
            {calc && (
              <div className="bg-bg-secondary rounded-2xl p-5 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold">{calc.baseCoins.toLocaleString(locale)} {t("token")}</span>
                </div>

                {calc.bonusPercent > 0 && (
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-accent-main font-medium">{t("bonus", { percent: calc.bonusPercent })}</span>
                    <span className="text-sm font-semibold text-accent-main">+{calc.bonusCoins.toLocaleString(locale)} {t("token")}</span>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-bold text-accent-main">{calc.totalCoins.toLocaleString(locale)} {t("token")}</span>
                </div>
              </div>
            )}
            ...
            <button
              disabled
              className="t-btn accept w-full !opacity-60 !cursor-not-allowed"
              aria-label={tCommon("betaNotice")}
            >
              {tCommon("betaNotice")}
            </button>
