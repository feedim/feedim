...
interface PeriodCounts { views: number; likes: number; comments: number; saves: number; shares: number; followers: number; profileVisits: number }
...
export default function AnalyticsPage() {
  ...
  const authorized = isLoggedIn ? ((ctxUser?.isPremium ?? false) || ctxUser?.role === "admin") : false;
  ...
  const getAnalyticsUrl = useCallback((currentPeriod: string) => (
    withCacheScope(`/api/analytics?period=${currentPeriod}&tz=${tz}`, ctxUser?.id ? `viewer:${ctxUser.id}` : "guest")
  ), [ctxUser?.id, tz]);

  useLayoutEffect(() => {
    if (!authorized) return;
    const cached = readCache(getAnalyticsUrl(period)) as {
      overview?: OverviewData;
      periodCounts?: PeriodCounts;
      prev?: PeriodCounts;
      viewsByDay?: DayData[];
      likesByDay?: DayData[];
      commentsByDay?: DayData[];
      followersByDay?: DayData[];
      peakHours?: HourData[];
      ...
      followerDemographics?: FollowerDemographicsData | null;
    } | null;

    if (!cached) return;
    setOverview(cached.overview || null);
    setPeriodCounts(cached.periodCounts || null);
    setPrev(cached.prev || null);
    setViewsByDay(cached.viewsByDay || []);
    setLikesByDay(cached.likesByDay || []);
    setCommentsByDay(cached.commentsByDay || []);
    setFollowersByDay(cached.followersByDay || []);
    setPeakHours(cached.peakHours || []);
    setWeekdayBreakdown(cached.weekdayBreakdown || []);
    setTopPosts(cached.topPosts || []);
    setAllPosts(cached.allPosts || []);
    setEarnings(cached.earnings || { coinBalance: 0, totalEarned: 0, periodEarned: 0, qualifiedReads: 0, premiumReads: 0 });
    setVideoAnalytics(cached.videoAnalytics || null);
    setFollowerDemographics(cached.followerDemographics || null);
    setLoading(false);
  }, [authorized, getAnalyticsUrl, period]);

  const loadAnalytics = useCallback(async (p: string) => {
    const analyticsUrl = getAnalyticsUrl(p);
    const cached = readCache(analyticsUrl);
    if (!cached) setLoading(true);
    try {
      const data = await fetchWithCache(analyticsUrl, { ttlSeconds: FRESHNESS_WINDOWS.analyticsOverview }) as {
        overview?: OverviewData;
        periodCounts?: PeriodCounts;
        prev?: PeriodCounts;
        viewsByDay?: DayData[];
      ...
      setOverview(data.overview ?? null); setPeriodCounts(data.periodCounts ?? null); setPrev(data.prev ?? null);
      ...
      <div className="pb-10">
        ...
        {loading ? (
          <div className="space-y-4 px-4">
            <div className="h-[100px] rounded-2xl bg-bg-secondary animate-pulse" />
            <div className="grid grid-cols-2 gap-3">
              <div className="h-[62px] rounded-2xl bg-bg-secondary animate-pulse" />
              <div className="h-[62px] rounded-2xl bg-bg-secondary animate-pulse" />
              <div className="h-[62px] rounded-2xl bg-bg-secondary animate-pulse" />
              <div className="h-[62px] rounded-2xl bg-bg-secondary animate-pulse" />
            </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-2.5 mx-4 mt-4">
              ...
              <MetricCard icon={Eye} label={t("profileVisits")} value={periodCounts?.profileVisits || 0} prev={prev?.profileVisits || 0} prevLabel={t("previousPeriod", { value: "" })} />
            </div>
    ...
    <div className="mx-4 mt-5">
      <div className="bg-bg-secondary rounded-[15px] p-4">
            ...
            return (
              <div key={i} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-text-primary text-bg-primary text-[10px] font-semibold px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition whitespace-nowrap pointer-events-none z-10 shadow-md">
                  ...
                  <span className="block text-center text-[8px] opacity-70">{new Date(d.date).toLocaleDateString(locale, { day: "numeric", month: "short" })}</span>
        ...
        <div className="flex justify-between mt-2.5">
          <span className="text-[10px] text-text-muted">{chartData.length > 0 && new Date(chartData[0].date).toLocaleDateString(locale, { day: "numeric", month: "short" })}</span>
          {chartData.length > 14 && <span className="text-[10px] text-text-muted">{new Date(chartData[Math.floor(chartData.length / 2)].date).toLocaleDateString(locale, { day: "numeric", month: "short" })}</span>}
          <span className="text-[10px] text-text-muted">{chartData.length > 0 && new Date(chartData[chartData.length - 1].date).toLocaleDateString(locale, { day: "numeric", month: "short" })}</span>
        </div>
      </div>
    </div>
  );
              ...
              {data.countries.map(c => {
                ...
                return (
                  <div key={c.code} className="flex items-center gap-2">
                    <span className="text-[0.72rem] font-medium min-w-[90px] w-[90px] truncate">{getCountryName(c.code)}</span>
...
function ContentTypeIcon({ type }: { type?: string }) {
  switch (type) {
    ...
    case "note": return <FileText className="w-5 h-5 text-text-muted" />;
