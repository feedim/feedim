...
export default function NotificationSettingsPage() {
  ...
  const toggleNotifPause = async () => {
    ...
    const prevPausedUntil = pausedUntil;
    ...
    try {
      ...
      if (!res.ok) {
        feedimAlert("error", t("genericError"));
        setNotifPaused(!newPaused);
        setPausedUntil(prevPausedUntil);
      } else {
        const result = await res.json();
        // Sync with server-confirmed values
        setNotifPaused(result.isPaused ?? newPaused);
        setPausedUntil(result.pausedUntil ?? newPausedUntil);
      }
    } catch {
      feedimAlert("error", t("genericError"));
      setNotifPaused(!newPaused);
