...
export default function WithdrawalPage() {
  ...
  const isPremiumAllowed = profile
    ? (isAdmin || ALLOWED_PLANS.includes(profile.premium_plan || "") || profile.is_premium || ALLOWED_PLANS.includes(currentUser?.premiumPlan || ""))
    : (isAdmin || ALLOWED_PLANS.includes(currentUser?.premiumPlan || ""));
  const isMfaEnabled = isAdmin || profile?.mfa_enabled || false;
  const hasIban = !!(profile?.withdrawal_iban && profile?.withdrawal_holder_name);
  const isMonetizationEnabled = isAdmin || profile?.monetization_enabled || false;
  const isProfessionalAccount = isAdmin || profile?.account_type === "creator" || profile?.account_type === "business";
  const isPrivateAccount = !isAdmin && (profile?.account_private || false);
  ...
  const handleSaveIban = async () => {
    ...
    if (mfaCode.length !== 6) {
      feedimAlert("error", t("mfaCodeRequired"));
      return;
    }
    setSavingIban(true);
    try {
      const [res] = await Promise.all([
        fetch("/api/withdrawal", {
          ...
          body: JSON.stringify({ iban, holder_name: holderName, mfa_code: mfaCode }),
        }),
  ...
  const hasPendingRequest = !isAdmin && requests.some(r => r.status === "pending" || r.status === "processing");
      ...
      <div className="py-4 px-3 sm:px-4 max-w-xl mx-auto space-y-5">
        {loading ? (
          <>
            ...
            <div className="h-[160px] rounded-2xl bg-bg-secondary animate-pulse" />
          </>
        ) : (
          <>
            {/* Mevcut Bakiye */}
            ...
            <div className={`bg-bg-secondary rounded-2xl p-5 ${(!isPremiumAllowed || !isMfaEnabled) ? "opacity-50 pointer-events-none" : ""}`}>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm text-text-muted mb-1.5">{t("mfaCodeLabel")}</label>
                  <DigitInput value={mfaCode} onChange={setMfaCode} disabled={hasPendingRequest} />
                </div>
                <button
                  onClick={handleSaveIban}
                  disabled={savingIban || !iban.trim() || !holderName.trim() || mfaCode.length !== 6 || hasPendingRequest}
            ...
            <div className={`bg-bg-secondary rounded-2xl p-5 ${(!isPremiumAllowed || !isMfaEnabled || !hasIban) ? "opacity-50 pointer-events-none" : ""}`}>
              ...
              <form onSubmit={handleSubmitWithdrawal} className="space-y-4">
                ...
                {balance >= COIN_MIN_WITHDRAWAL && (
                  ...
                  >
                    {t("withdrawAll", { amount: balance.toLocaleString(locale) })}
                  </button>
                )}
