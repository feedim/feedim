...
export default async function EmbedPage({ params }: PageProps) {
  ...
  const tCT = await getTranslations("contentTypes");
  const locale = await getLocale();

  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://feedim.com";
  const contentPrefix = post.content_type === "video" ? "/video" : post.content_type === "note" ? "/note" : post.content_type === "moment" ? "/moments" : "";
  const postUrl = `${baseUrl}${contentPrefix}/${post.slug}`;
    ...
    return (
      <div className="w-full h-full bg-black flex items-center justify-center overflow-hidden">
        ...
        <div className="flex-1 min-h-0 relative">
          ...
          <div className="absolute bottom-10 left-3 right-12 z-[7] pointer-events-none">
            <div className="flex items-center gap-2 mb-1">
              {author?.avatar_url ? (
                <img suppressHydrationWarning data-src={author.avatar_url} alt="" className="lazyload h-7 w-7 rounded-full object-cover shrink-0 bg-bg-tertiary border border-border-primary" />
              ) : (
        ...
        >
          ...
          <span className="text-white/90 text-[0.78rem] font-semibold truncate">{tCT("moment")}</span>
          ...
          <article className="pt-[4px] pb-[9px] pl-3 pr-3.5">
            <div className="flex gap-2 items-stretch">
              <div className="shrink-0 w-[42px] pt-[11px] pb-0 flex flex-col items-center">
                {author?.avatar_url ? (
                  <img suppressHydrationWarning data-src={author.avatar_url} alt={author?.username || ""} className="lazyload h-[42px] w-[42px] min-w-[42px] max-w-[42px] rounded-full object-cover bg-bg-tertiary relative z-[1]" />
                ) : (
                ...
                <div className="flex items-center gap-1 min-w-0">
                  ...
                  {post.published_at && (
                    <>
                      ...
                      <span style={{ fontSize: "0.62rem", color: "#6b7280" }} className="shrink-0">{formatRelativeDate(post.published_at, locale)}</span>
                    </>
                  )}
                  <span style={{ color: "#9ca3af80", fontSize: "0.75rem" }}>·</span>
          ...
          <div className="flex items-center gap-2 px-3 mt-3">
            {(post.comment_count ?? 0) > 0 && (
              <div className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5" style={{ borderRadius: 12, background: "#f3f4f6", fontSize: "0.82rem", fontWeight: 500, color: "#6b7280" }}>
                ...
                <span>{formatCount(post.comment_count, locale)}</span>
              </div>
            )}
            {(post.like_count ?? 0) > 0 && (
              <div className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5" style={{ borderRadius: 12, background: "#f3f4f6", fontSize: "0.82rem", fontWeight: 500, color: "#6b7280" }}>
                ...
                <span>{formatCount(post.like_count, locale)}</span>
              </div>
            )}
          </div>
        </a>
        ...
        <article className="pt-[4px] pb-[9px] pl-3 pr-3.5">
          <div className="flex gap-2 items-stretch">
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-0 p-[5px]">
              ...
              {/* View count */}
              {(post.view_count ?? 0) > 0 && (
                <span style={{ fontSize: "0.7rem", color: "#6b7280", marginTop: 4 }}>{t("viewCount", { count: formatCount(post.view_count, locale) })}</span>
              )}
            </div>
          </div>
        </article>
