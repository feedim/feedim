...
export default async function AccountModerationPage() {
  ...
  // For active accounts: also fetch previous non-approved decision to show original reason
  let previousReason: string | null = null;
  let previousDecisionType: string | null = null;
  if (profile.status !== "frozen") {
    ...
    // If last decision is approved, fetch previous decision to show original block/moderation reason
    if (decisionType === 'approved') {
      const { data: prevDecision } = await admin
        .from("moderation_decisions")
        ...
        .limit(1)
        .single();
      previousReason = prevDecision?.reason || null;
      previousDecisionType = prevDecision?.decision || null;
    }
  }
            ...
            <div className="bg-bg-secondary rounded-[30px] p-5 space-y-4">
              ...
              {previousReason && (
                <div className="bg-[var(--accent-color)]/5 rounded-lg p-3">
                  <p className="text-xs font-medium text-[var(--accent-color)]">{t("reasonLabel")}</p>
                  <p className="text-sm text-text-primary mt-0.5 font-semibold">{previousReason}</p>
                </div>
              )}
              {decisionReason && (
                <div className="bg-success/5 rounded-lg p-3">
                  ...
                  {decisionDate && (
                    <p className="text-[0.68rem] text-text-muted mt-1">{new Date(decisionDate).toLocaleDateString(locale, { day: "numeric", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" })}</p>
                  )}
                </div>
              )}
              <Link
        ...
        <div className="max-w-lg w-full space-y-6">
          ...
          <div className="bg-bg-secondary rounded-[30px] p-5 space-y-4">
            ...
            {isDeleted && profile.updated_at && (() => {
              const remaining = getRemainingDeletionDays(profile.updated_at);
              return (
